import { processNextOutreachDiscoveryJob } from "../src/features/outreach/worker";
...
const workerId = `outreach-discovery-worker-${process.pid}-${Date.now()}`;
let shouldStop = false;

process.on("SIGINT", () => {
    shouldStop = true;
    console.log("\nOutreach discovery worker received SIGINT. Stopping after current iteration...");
});
process.on("SIGTERM", () => {
    shouldStop = true;
    console.log("\nOutreach discovery worker received SIGTERM. Stopping after current iteration...");
});

async function main() {
    console.log(`Starting Outreach Discovery Worker (${workerId})...`, options);
    let processed = 0;
    let failed = 0;
    ...
    await reportWorkerHealth({
        ...
        status: "healthy",
        message: "Outreach discovery worker daemon started.",
    ...
    while (!shouldStop) {
        ...
        let result;
        try {
            result = await processNextOutreachDiscoveryJob(workerId);
        } catch (err) {
            const message = err instanceof Error ? err.message : String(err);
            failed += 1;
            console.error(`[outreach-discovery-worker] Unexpected loop error: ${message}`);
            await reportWorkerHealth({
                provider: "outreach",
                integrationKey: "discovery-worker",
                status: "degraded",
        ...
        if (!result.success && result.message === "No queued jobs found.") {
            ...
            if (!wasIdle || now - lastIdleWriteAt >= HEARTBEAT_INTERVAL_MS) {
                ...
                await reportWorkerHealth({
                    ...
                    integrationKey: "discovery-worker",
                    status: "healthy",
                    message: "No queued discovery jobs found; worker is idle.",
                    details: { workerId, idle: true, processed, failed }
                });
            }
            if (!options.daemon) break;
            console.log(`No queued discovery jobs found. Sleeping for ${options.idleDelayMs}ms...`);
            await sleep(options.idleDelayMs);
            continue;
        }
        ...
        console.log(`[outreach-discovery-worker] ${result.success ? "Processed" : "Failed"} ${result.jobId ?? "no-job"}: ${result.message}`);
        ...
        if (now - lastHeartbeatAt >= HEARTBEAT_INTERVAL_MS) {
            ...
            await reportWorkerHealth({
                ...
                integrationKey: "discovery-worker",
                status: "healthy",
                message: "Outreach discovery worker daemon heartbeat.",
                details: { workerId, processed, failed }
            });
        }
    ...
    console.log(`Outreach Discovery Worker finished. Processed ${processed}. Failed/skipped ${failed}.`);
}
...
main().catch(async (error) => {
    ...
    try {
        await reportWorkerHealth({
            ...
            integrationKey: "discovery-worker",
            status: "failing",
            message: `Fatal error in outreach-discovery-worker: ${message}`,
            details: { workerId }
        });
    } catch (healthErr) {
