...
import type { SeoContentPlanRecord } from "@/features/seo/types";
import { bulkDeleteSeoContentPlans } from "@/features/seo/actions";
import { BulkActionButton, BulkActionToolbar, SelectionCheckbox } from "@/shared/ui/list-controls";

interface StrategistPlansPanelProps {
    items: SeoContentPlanRecord[];
    /** Pre-rendered row nodes, one per item, in the same order as `items`. */
    renderedRows: ReactNode[];
}

export function StrategistPlansPanel({ items, renderedRows }: StrategistPlansPanelProps) {
    ...
    const isSelectable = (item: SeoContentPlanRecord) =>
        !item.draft_content_item_id && item.status !== "in_progress" && item.status !== "done";
    ...
    const bulkDelete = () => {
        ...
        if (
            !confirm(
                `Delete ${ids.length} plan${ids.length === 1 ? "" : "s"}? Plans with a draft, or in progress / done, will be skipped.`,
            )
        )
            return;
        setActionError(null);
        setActionInfo(null);
        startBulk(async () => {
            const res = await bulkDeleteSeoContentPlans(ids);
            if (res.error) {
                setActionError(res.error);
                return;
            }
            const parts: string[] = [];
            parts.push(`Deleted ${res.deleted} plan${res.deleted === 1 ? "" : "s"}.`);
            if (res.skipped > 0) {
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 rounded-md border border-border/60 bg-card/40 px-4 py-3 text-[15px]">
                <div className="inline-flex items-center gap-2 text-muted-foreground">
                    <SelectionCheckbox
                        ...
                        label="Select all deletable content plans on this page"
                    />
                    <span>
                        {selected.size === 0
                            ? "Select"
                ...
                {selectableIds.length < items.length ? (
                    <span className="text-muted-foreground">
                        {items.length - selectableIds.length} drafted/active row{items.length - selectableIds.length === 1 ? "" : "s"} excluded
                    </span>
                ) : null}
            </div>
                    ...
                    <div key={item.id} className="relative">
                        ...
                        >
                            <SelectionCheckbox
                                ...
                                label={`Select content plan ${item.title}`}
                                title={selectable ? "Select for bulk actions" : "Plan has a draft or active status — remove the draft / reset status first"}
                                size="sm"
                            />
                        </div>
                        {renderedRows[index]}
